                    ...
                    keywordViewModel.currentMonthRatio.collectLatest {
                        when (it) {
                            is MainUiState.success -> {
                                ...
                                keywordCnt = periodRadioData.get(0).ratioData?.map {
                                    it.rate?.toDouble() ?: 0.0
                                } ?: emptyList()
                                keywordPeriod =
                                    periodRadioData.get(0).ratioData?.map { it.period.toString() }
                                        ?: emptyList()
                                lineChartDrawer.setChart(
                                    binding.keywordLineChart,
                                    requireContext(),
                                    keywordCnt,
                                    keywordPeriod
                                )
//
                        ...
                        when (it) {
                            is MainUiState.success -> {
                                binding.apply {
                                    Log.d(
                                        "hhh",
                                        "KeywordFragment - onCreateView() - called ${it.data}"
                                    )
                                    ...
                                    currentMonthCnt =
                                        (it.data.get(0).monthlyPcQcCnt!!.toInt() + it.data.get(0).monthlyMobileQcCnt!!.toInt()).toString()
                                }
                            }
        ...
        binding.keywordLineChart.setOnChartValueSelectedListener(object :
            OnChartValueSelectedListener {
            override fun onValueSelected(e: Entry, h: Highlight) {
                val xAxisLabel = e.x.let {
                    binding.keywordLineChart.xAxis.valueFormatter.getAxisLabel(
                        it,
                        binding.keywordLineChart.xAxis
                    )
                }
