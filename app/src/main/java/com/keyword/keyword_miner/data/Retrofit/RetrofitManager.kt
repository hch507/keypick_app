...
import com.keyword.keyword_miner.utils.*
...
class RetrofitManager {
    ...
    private var iRetrofit : IRetrofit? = RetrofitClient.getRetrifitClient(API.BASE_URL)
        ?.create(IRetrofit::class.java)
    private var iRetrofit_search : IRetrofit? = RetrofitClient.getRetrifitClient(Search_API.BASE_URL)
        ?.create(IRetrofit::class.java)
    private var iRetrofit_blog : IRetrofit? = RetrofitClient.getRetrifitClient(Blog_API.BASE_URL)
        ?.create(IRetrofit::class.java)

    private var iRetrofit_blog_data : IRetrofit? = RetrofitClient.getGsonRetrifitClient(MY_BLOG.MY_BASE_URL)
        ?.create(IRetrofit::class.java)
